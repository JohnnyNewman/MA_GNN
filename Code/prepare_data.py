...
def get_data_from_simulation(
    ...
    n_vol_samples = int(n_vol_sample_multiplier * len(mesh.cells[0]))
    print(n_vol_samples)

    x_vol, _ = sample_points(
        mesh.cells[0].data, mesh.points, n_vol_samples, replace=False
    )
    ...
    # airfoil
    m1 = x_res_bc_type == 1
    u_res_bc_mask[m1, 1:3] = n[x_res_bc_sample_inds[m1]]
    u_res_bc[m1, 2] = 0.0

    # lower_wall --> y-velocity == 0
    m2 = x_res_bc_type == 2
    u_res_bc_mask[m2, 2] = True
    u_res_bc[m2, 2] = 0.0

    # inlet --> x-velocity == 1.775, y-velocity == 0
    m3 = x_res_bc_type == 3
    u_res_bc_mask[m3, 1] = True
    u_res_bc[m3, 1] = 1.0  # 1.775
    u_res_bc_mask[m3, 2] = True
    u_res_bc[m3, 2] = 0.0

    # outlet --> pressure = 0
    m4 = x_res_bc_type == 4
    u_res_bc_mask[m4, 0] = True
    u_res_bc[m4, 0] = 0.0

    # upper_wall --> y-velocity == 0
    m5 = x_res_bc_type == 5
    u_res_bc_mask[m5, 2] = True
    u_res_bc[m5, 2] = 0.0
